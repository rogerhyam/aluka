<?php
    ...
    */
    function doDownload($project, $download){
        ...
        $sql = "
            ...
            ALTER TABLE mobile.my_names ADD INDEX(name_num);
             ...
             SELECT
                     ...
                     s.coll_name as specimen_filing_name_free,
                     s.coll_num as specimen_collector_number,
                     
                     s.gard_collector as specimen_garden_collector_name,
                     s.gard_coll_num as specimen_garden_collector_number,
        ...
        // they come sorted by specimen number and specimens may rep
        while ($row = $response->fetch_assoc()) {
           ...
           */
            ...
            */
            ...
            // get the names associated with being types.
            $sql = "SELECT
            
             nomen_type_num as type_name_num,
             case nomen_type_kind
               ...
               FROM types 
               WHERE specimen_num = $specimenNumber ";
            ...
            $typesResults = $mysqli->query($sql);
             ...
             while ( $type = $typesResults->fetch_assoc() ) {
                 
                  if(!$type['type_name_num']) continue; // only do it if there is a name number
                 
                  $identification = $xml->createElement('Identification');
                  $identification->appendChild($xml->createComment(' From type_name_num '));
                  $unitXML->appendChild($identification);

                  $typeKind = $type['type_kind'];
                  if(!$typeKind) $typeKind = 'Type';
                  
                  writeIdentification($xml, $identification, $type['type_name_num'], false, $typeKind);
            }


            // In cases whre the gard_collector field is not null we use these 
            // fields in preference to the main collector fields
            if($row['specimen_garden_collector_name']){

                if($row['specimen_garden_collector_name']){
                    $unitXML->appendChild( $xml->createElement('Collectors', xmlSafe(  $row['specimen_garden_collector_name'] ) ) );
                }

                if($row['specimen_garden_collector_number']){
                    $unitXML->appendChild( $xml->createElement('CollectorNumber', xmlSafe(  $row['specimen_garden_collector_number']  ) ) );
                }else{
            ...
            */
            ...
            $unitXML->appendChild( $xml->createElement('Notes', 'http://data.rbge.org.uk/herb/' . $row['specimen_barcode'] ) );
        
            // we have created the $unitXML and it contains identifications from everywhere - more than one of them may
            // have StoredUnderName="true" - but only one should have this value.
            // so work through the XML and fix it.
            
            $idents = $unitXML->getElementsByTagName('Identification');
            $alreadyHadTrue = false;
            foreach ($idents as $id){
                
                if($id->getAttribute('StoredUnderName') == 'true'){
    
                    if($alreadyHadTrue == 'true'){
                        $id->setAttribute('StoredUnderName','false');
                    }else{
                        $alreadyHadTrue = true;
                    }
         ...
         if($download){
            ...
            $filename = "E_" . $project . "_" . date('Ymd') . '.xml';
...
<?php
                ...
                while ($row = $response->fetch_assoc()) {
                    ...
                    $display = 'Project code: ' . $projectName . " (" . $row['n'] . ")";
